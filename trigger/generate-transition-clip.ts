...
export const generateTransitionClipTask = task({
  ...
  run: async (payload: GenerateTransitionClipPayload) => {
    ...
    try {
      ...
      logger.info("Generating transition clip", { clipId, fromImageUrl, toImageUrl })
      ...
      logger.info("Uploaded transition images to Fal.ai storage", { falFromUrl, falToUrl })

      // Step 3: Generate transition prompt
      const transitionPrompt = "Smooth, seamless morphing transition between two scenes. Professional cinematic blend with natural motion. Elegant and fluid transformation."
